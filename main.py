...
def point_in_rect(point: tuple[int, int], rect: pygame.Rect) -> bool:
    ...
    :param rect: The rectangle that might hold the point.
    :return: True if the point is in the rect, else false
    """
...
async def main():
    ...
    rebirths_display = lambda rebirths: Text('rebirths_display', f"Rebirths: {rebirths}", white, black, 64, WIDTH // 2,
                                             96)
    sprites.append(rebirths_display(0))
    prestige_display = lambda prestige: Text('prestige_display', f"Prestiges: {prestige}", white, black, 64, WIDTH // 2,
                                             160)
    sprites.append(rebirths_display(0))
    ...
    while game_loop:
        ...
        sprites = [prestige_display(prestiges), prestige(prestige_cost), rebirths_display(rebirths),
                   score_display(score),
                   upgrade(upgrade_cost), rebirth(rebirth_cost)]
        show(sprites)
